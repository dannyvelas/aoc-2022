...
(defn top-n [tally-map n]
  (cond (= n 0) []
        :else (let [[-max-key freq] (apply max-key key tally-map)
                    new-map (cond (= freq 1) (dissoc tally-map -max-key)
                                  :else (assoc tally-map -max-key (dec freq)))]
                (into [] (concat [-max-key] (top-n new-map (dec n)))))))

(defn tally
  ([v] (tally v {} 0))
  ([v as-map i]
   (cond (< i (count v)) (let [e (get v i)
